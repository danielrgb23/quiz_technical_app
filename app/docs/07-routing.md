...
Routes can receive parameters via the URL path. This is useful for detail screens where you need an ID or slug.

### 1. Define the route with a parameter placeholder in `AppRoutes`
...
- `homeDetail` is the **template** used in the router definition (contains `:id`)
- `homeDetailPath()` is the **helper** used for navigation (builds the actual path)

### 2. Receive the parameter in the page
...
The `@PathParam('id')` annotation tells auto_route to extract the `:id` segment from the URL and inject it into the constructor.

### 3. Navigate with the helper method

```dart
// From any feature — no cross-feature imports needed
context.router.pushPath(AppRoutes.homeDetailPath(item.id));
// Navigates to /home/abc-123
```
...
|---|---|---|
| URL format | `/home/abc-123` | `/home?id=abc-123` |
| Definition | `/home/:id` | `/home` |
| Annotation | `@PathParam('id')` | `@QueryParam('id')` |
| Best for | Required identifiers | Optional filters/flags |
| Example | Detail pages, user profiles | Search filters, pagination |

### Query Parameters (reference)

Query parameters work similarly but use `@QueryParam()`:
...
Navigate with: `context.router.pushPath('/search?q=flutter&page=2')`
